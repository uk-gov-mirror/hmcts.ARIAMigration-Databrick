...
@dlt.table(
    name="stg_uppertribunal_filtered",
    comment="Delta Live Table for filtering second tier AppealCase records for archive or delete based on complex conditions.",
    path=f"{bronze_mnt}/stg_uppertribunal_filtered"
)
def stg_uppertribunal_filtered():
    appeal_cases =  dlt.read("stg_appealcasestatus_filtered")
    
    UTRetained_cases = appeal_cases.alias('ac').filter(col("CaseStatusCategory") == 'UTA').select("ac.CaseNo", lit('ARIAUTA').alias('Segment'))
...
def stg_appeals_filtered():
    ...
    stg_uppertribunalretained_filtered =  dlt.read("stg_uppertribunal_filtered")
...
def silver_archive_metadata():
    ...
    df_StatusOutcomeCombinations_uta = (
        df_StatusOutcomeCombinations.filter((col("Case Type") == "Appeal") & (col("Tier") == "UT")
            ).select(
    ...
    comparison_df = (
        metadata_df.alias("b")
        .join(
            df_StatusOutcomeCombinations_uta.alias("uta"),
            (col("b.CaseStatus") == col("uta.lk_case_status")) &
            (col("b.Outcome") == col("uta.lk_outcome")),
            how="left"
        )
        .withColumn(
            "is_valid_status_outcome",
            F.when(col("uta.lk_case_status").isNotNull(), lit(True))
            .otherwise(lit(False))
        )
